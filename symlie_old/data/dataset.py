...
class MnistDataset(Dataset):
    ...
    def __init__(self, mode: str, dim: int = 28, augment: str = 'none', augment_kwargs: dict = {}, N: int = -1, data_dir = '../data/mnist'):
        ...
        if self.dim != 28:
            resize = Resize(dim)
            self.images = resize(self.images.unsqueeze(1)).squeeze(1)
    ...
    def space_translation(self, img, shift = None):
        shift = int(torch.randint(0, self.dim, (1,))) if shift is None else shift
...
class FlatDataset(Dataset):
    ...
    def __init__(self, mode: str, augment: str = 'none', data_kwargs: dict = {}, N: int = -1, data_dir = '../data/flat'):
        assert mode in ['train', 'test', 'val']

        self.augment = augment
        self.data_kwargs = data_kwargs

        x = np.load(os.path.join(data_dir, mode, f'randn.npy'))
        _, self.dim = x.shape

        N = x.shape[0] if N == -1 else N

        self.x = torch.from_numpy(x[:N])
    ...
    def __getitem__(self, index):
        ...
        if self.augment == 'none':
            # transf_func = self.none
            eps = torch.tensor([0.])
        elif self.augment == 'space_translation':
            eps = torch.rand((1,))
            # transf_func = self.space_translation
        else:
    ...
    def space_translation(self, img, shift = None):
        shift = int(torch.randint(0, self.dim, (1,))) if shift is None else shift
        img = torch.roll(img, shifts=shift, dims=0)
        return img
...
class FlatDataModule(pl.LightningDataModule):
    ...
    def custom_collate(self, batch):
        # x, transf_func = zip(*batch)
        # t = [transf_func_i for transf_func_i in transf_func]
        # return torch.stack(x), t

        x, eps = zip(*batch)
        return torch.stack(x), torch.stack(eps)
...
class FlatYDataset(Dataset):

    def __init__(self, mode: str, augment: str = 'none', 
                 data_kwargs: dict = {
                    'space_length': 4,
                    'noise_std': 0.1,
                    'y_low': 0.5,
...
class FlatYDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            augment,
            data_kwargs,
    ...
    def custom_collate(self, batch):
        ...
        x, y, eps = zip(*batch)
        return torch.stack(x), torch.stack(y), torch.stack(eps)
