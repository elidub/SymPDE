...
def printshape(*args):
    print(*tuple(map(lambda x: x.shape, (args))))

def printcustom(f, *args):
    print(*tuple(map(f, (args))))
...
class SyntheticLoader:
    ...
    def next(self, n_tasks, mode="train"):
        ...
        # printshape(task_idcs, self.xs, self.ys, self.ws)
        ...
        for i in range(n_tasks):
            ...
            # printcustom(lambda x: len(x), example_idcs,spt_idcs, qry_idcs)
