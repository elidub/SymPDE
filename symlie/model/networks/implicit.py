...
class LinearImplicit(nn.Module):
    ...
    def forward(self, x, batch_size = None, return_weight = False):
        ...
        if self.train_weights or (batch_size is None):
            ...
            weight = self.implicit_P(weight)
            weight = weight.view(self.in_features, self.out_features)
            out = x @ weight.T

        else:
            weight = self.weight
            weight = weight.view(batch_size, self.in_features*self.out_features)
            weight = self.implicit_P(weight)
            weight = weight.view(batch_size, self.in_features, self.out_features)
...
# class LinearImplicit_Intermediate(nn.Module):
#     def __init__( 
#             self, in_features, out_features, bias, 
#             hidden_implicit_layers: List[int],
#             device = 'cpu',
...
#         # self.features = self.in_features * self.out_features
...
#         # if P_init is not None:
#         #     print('Loading weights P')
#         #     self.layers.load_state_dict(P_init)
            
#     def reset_parameters(self, batch_size = None) -> None:
#         self.weight = torch.randn(self.in_features, self.out_features, device = self.device)

#     def setup_implict_P(self, hidden_implicit_layers: List[int]):
#         implicit_in_features = implicit_out_features = self.in_features*self.out_features
#         # implicit_P = nn.Sequential(
#         #     nn.Linear(implicit_in_features,implicit_in_features),
#         #     nn.ReLU(),
#         #     nn.Linear(implicit_in_features,implicit_in_features),
#         #     nn.ReLU(),
#         #     nn.Linear(implicit_in_features,implicit_in_features),
...
class LinearImplicit_Original(nn.Module):
    def __init__( 
            self, in_features, out_features, bias, 
            hidden_implicit_layers: List[int],
            device = 'cpu',
